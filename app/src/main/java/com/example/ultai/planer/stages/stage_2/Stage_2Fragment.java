...
public class Stage_2Fragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (backButton != null) {
            backButton.setOnClickListener(v -> {
                ...
                navController.navigate(R.id.action_faza2_stages_to_navigation_planer);
            });
        }
        ...
        if (profileButton != null) {
            profileButton.setOnClickListener(v -> {
                NavController navController = Navigation.findNavController(v);
                navController.navigate(R.id.action_faza2_stages_to_profileFragment);
            });
        }
    ...
    /**
     * Инициализация UI элементов для шагов этапа 2
     */
    private void initializeStepsUI(View view) {
        ...
        // Инициализация текстов шагов для этапа 2
        setupStepTexts();
    }
    ...
    /**
     * Настройка текстов для шагов этапа 2
     */
    private void setupStepTexts() {
        try {
            // Устанавливаем заголовки и описания для шагов этапа 2
            setStepText(step1Container, R.id.step_1_title, "Шаг 1");
            setStepText(step1Container, R.id.step_1_desc, "Какие продукты/услуги вы предлагаете");
            
            setStepText(step2Container, R.id.step_2_title, "Шаг 2");
            setStepText(step2Container, R.id.step_2_desc, "Каковы текущие объемы продаж продуктов/услуг");
            
            setStepText(step3Container, R.id.step_3_title, "Шаг 3");
            setStepText(step3Container, R.id.step_3_desc, "Каковы текущие цены на ваши продукты или услуги");
            
            setStepText(step4Container, R.id.step_4_title, "Шаг 4");
            setStepText(step4Container, R.id.step_4_desc, "Каковы основные каналы продаж");
            
            setStepText(step5Container, R.id.step_5_title, "Шаг 5");
            setStepText(step5Container, R.id.step_5_desc, "Какие ресурсы доступны вам для реализации планов по увеличению продаж");
            
            setStepText(step6Container, R.id.step_6_title, "Шаг 6");
            setStepText(step6Container, R.id.step_6_desc, "Какова общая структура доходов и расходов");
            
            if (step7Container != null) {
                setStepText(step7Container, R.id.step_7_title, "Шаг 7");
                setStepText(step7Container, R.id.step_7_desc, "Какие проблемы или вызовы вы испытываете в настоящее время в своем бизнесе");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void setupStepClickListeners() {
        ...
        if (step1Container != null) {
            step1Container.setOnClickListener(v -> {
                ...
                navController.navigate(R.id.action_faza2_stages_to_step1Stage2Fragment);
            });
        }
        ...
        if (step2Container != null) {
            step2Container.setOnClickListener(v -> {
                if (completedSteps[0]) {
                    NavController navController = Navigation.findNavController(requireView());
                    navController.navigate(R.id.action_faza2_stages_to_step2Stage2Fragment);
                } else {
        ...
        if (step3Container != null) {
            step3Container.setOnClickListener(v -> {
                if (completedSteps[0] && completedSteps[1]) {
                    NavController navController = Navigation.findNavController(requireView());
                    navController.navigate(R.id.action_faza2_stages_to_step3Stage2Fragment);
                } else {
        ...
        if (step4Container != null) {
            step4Container.setOnClickListener(v -> {
                if (completedSteps[0] && completedSteps[1] && completedSteps[2]) {
                    NavController navController = Navigation.findNavController(requireView());
                    navController.navigate(R.id.action_faza2_stages_to_step4Stage2Fragment);
                } else {
                    Toast.makeText(requireContext(), "Сначала выполните предыдущие шаги", Toast.LENGTH_SHORT).show();
                }
            });
        ...
        if (step5Container != null) {
            step5Container.setOnClickListener(v -> {
                if (completedSteps[0] && completedSteps[1] && completedSteps[2] && completedSteps[3]) {
                    NavController navController = Navigation.findNavController(requireView());
                    navController.navigate(R.id.action_faza2_stages_to_step5Stage2Fragment);
                } else {
                    Toast.makeText(requireContext(), "Сначала выполните предыдущие шаги", Toast.LENGTH_SHORT).show();
                }
            });
        ...
        if (step6Container != null) {
            step6Container.setOnClickListener(v -> {
                if (completedSteps[0] && completedSteps[1] && completedSteps[2] && completedSteps[3] && completedSteps[4]) {
                    NavController navController = Navigation.findNavController(requireView());
                    navController.navigate(R.id.action_faza2_stages_to_step6Stage2Fragment);
                } else {
                    Toast.makeText(requireContext(), "Сначала выполните предыдущие шаги", Toast.LENGTH_SHORT).show();
                }
            });
        ...
        if (step7Container != null) {
            step7Container.setOnClickListener(v -> {
                if (completedSteps[0] && completedSteps[1] && completedSteps[2] && completedSteps[3] && completedSteps[4] && completedSteps[5]) {
                    NavController navController = Navigation.findNavController(requireView());
                    navController.navigate(R.id.action_faza2_stages_to_step7Stage2Fragment);
                } else {
                    Toast.makeText(requireContext(), "Сначала выполните предыдущие шаги", Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void checkStepsCompletionStatus() {
        ...
        SharedPreferences prefs = requireContext().getSharedPreferences("steps_completion_stage2", 0);
        
        // Проверяем все шаги по порядку
        completedSteps[0] = prefs.getBoolean("stage2_step1_completed", false);
        completedSteps[1] = prefs.getBoolean("stage2_step2_completed", false);
        completedSteps[2] = prefs.getBoolean("stage2_step3_completed", false);
        completedSteps[3] = prefs.getBoolean("stage2_step4_completed", false);
        completedSteps[4] = prefs.getBoolean("stage2_step5_completed", false);
        completedSteps[5] = prefs.getBoolean("stage2_step6_completed", false);
        completedSteps[6] = prefs.getBoolean("stage2_step7_completed", false);
    ...
    private void updateStepsUI() {
        ...
        int completedStepBackground = getResources().getColor(R.color.yellow, null);
